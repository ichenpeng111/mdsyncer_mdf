...
def result_analyse():
    ...
    for item in tasklist:
        print('%s\t%s.%s\t%s\t%s\t% 4d\t% 2.1f%%\t% .2f' % (item['stat'], item['host'], item['path'],
                                                            ...
                                                            (('D' == item['stat']) and time.strftime('%H:%M:%S',
                                                                                                     time.localtime(
                                                                                                         item[
                                                                                                             'done']))) or '--',
    ...
    print(
        '%d task(s) on %d server(s), Total elapsed %d second(s), %.2f second(s) per task in average' % (statvars['cnt'],
                                                                                                        statvars['svr'],
                                                                                                        statvars['sum'],
                                                                                                        float(statvars[
                                                                                                                  'sum']) /
    ...
    print(
        '%d task(s) done odps comit, Total elapsed %d second(s), %.2f second(s) per task in average, tilt index: %.2f' % (
            statvars_commit['cnt'],
